...
const Properties = () => {
  ...
  // Always start from page 1 when component mounts
  useEffect(() => {
    setCurrentPage(1);
  }, []);
  ...
  useEffect(() => {
    ...
    setSearchParams(prev => ({
      ...prev,
      propertyType: type || '',
      category: category || 'residential', // Default to residential
      locationId: loc || '',
      city: city ? (() => {
      ...
      maxPrice: budget ? parseInt(budget) : 500000000,
      status: status || '',
      beds: beds || '',
      bedrooms: bedrooms || '',
      bathrooms: bathrooms || '',
      minArea: minArea ? parseInt(minArea) : 0,
      maxArea: maxArea ? parseInt(maxArea) : 100000,
      landmark: landmark || '',
      developer: developer || '',
...
let filteredProperties = allProperties.filter(property => {
  ...
  let filteredProperties = propertiesData.filter(property => {
    ...
    if (searchParams.search && searchParams.search.trim() !== '') {
      ...
      const developerData = developersData.find(dev => dev.id === property.developerId);
      ...
      const searchFields = [
        ...
        developerData?.name?.toLowerCase() || '',
        property.propertyType?.toLowerCase() || ''
      ];
      
      // More flexible search - match any part of any field
      const matchesSearch = searchFields.some(field => {
        ...
        return searchWords.some(word => field.includes(word));
      });
    ...
    // Category filter (residential or commercial)
    if (searchParams.category && searchParams.category !== '') {
      if (property.category !== searchParams.category) {
        return false;
      }
    }
    ...
    // Status filter
    if (searchParams.status && searchParams.status !== '' && searchParams.status !== 'any-status') {
    ...
    // Developer filter
    if (searchParams.developer && searchParams.developer !== '' && searchParams.developer !== 'all-developers') {
      const developerData = developersData.find(dev => dev.id === property.developerId);
      if (!developerData || developerData.name.toLowerCase() !== searchParams.developer.toLowerCase()) {
        return false;
      }
    }
    ...
    if (searchParams.maxArea < 100000 && (property.area || 0) > searchParams.maxArea) {
      return false;
    }
    ...
    // Features & Amenities filter (unified - checks against both property.features and property.amenities)
    if (searchParams.features.length > 0) {
      const hasAllFeatures = searchParams.features.every(selectedFeature => {
        // Ensure property arrays exist, default to empty arrays if undefined
        const propertyFeatures = property.features ?? [];
        const propertyAmenities = property.amenities ?? [];
        
        // Check if the selected feature exists in either property.features OR property.amenities
        const inFeatures = propertyFeatures.some(propFeature => 
          propFeature.toLowerCase().includes(selectedFeature.toLowerCase())
        );
        const inAmenities = propertyAmenities.some(propAmenity => 
          propAmenity.toLowerCase().includes(selectedFeature.toLowerCase())
        );
        return inFeatures || inAmenities;
      });
      if (!hasAllFeatures) return false;
    }
    
    // Legacy amenities filter (for backward compatibility with URL params)
  ...
  const hasActiveFilters = () => {
    return searchParams.propertyType !== '' || 
           ...
           searchParams.maxPrice < 500000000;
  };
  ...
  return (
    ...
    >
          ...
          <div className="lg:col-span-1 hidden lg:block">
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="space-y-6">
                    ...
                    <SelectContent>
                      ...
                      {searchParams.city === 'rasalkhaimah' && (
                        <>
                          ...
                          <SelectItem value="bab-al-bahr">Bab Al Bahr</SelectItem>
                          <SelectItem value="pacific">Pacific</SelectItem>
                          <SelectItem value="dafan-al-nakheel">Dafan Al Nakheel</SelectItem>
                        </>
                      )}
                      {searchParams.city === 'fujairah' && (
                        <>
                          <SelectItem value="fujairah-creative-city">Fujairah Creative City</SelectItem>
                          <SelectItem value="fujairah-beach">Fujairah Beach</SelectItem>
                          <SelectItem value="dibba">Dibba (select projects)</SelectItem>
                        </>
                      )}
                      {searchParams.city === 'ummalquwain' && (
                        <>
                          <SelectItem value="emirates-modern-industrial-area">Emirates Modern Industrial Area</SelectItem>
                          <SelectItem value="al-salamah">Al Salamah</SelectItem>
                          <SelectItem value="al-ramlah">Al Ramlah</SelectItem>
                          <SelectItem value="al-maidan">Al Maidan</SelectItem>
                        </>
                      )}
                ...
                {/* Price Range */}
                <div>
                  <label className="text-sm font-medium mb-2 block">Price Range (<DirhamLogo className="inline" size="sm" />)</label>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span><DirhamLogo className="inline" size="xs" /> 0</span>
                    <span>Up to <DirhamLogo className="inline" size="xs" /> {searchParams.maxPrice.toLocaleString()}</span>
                  </div>
                </div>
                        ...
                        <AccordionContent>
                          <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                            {category.items.map((feature, featureIndex) => {
                              const featureId = `feature-${categoryIndex}-${featureIndex}`;
                              return (
                        ...
                        <div className="border-t border-gray-100 pt-4">
                          <div className="flex justify-between text-gray-600">
                            <div className="flex items-center">
                              ...
                              <span>{formatBedrooms(property.beds)}{formatBedrooms(property.beds) === 'Studio' ? '' : ' Beds'}</span>
                            </div>
                            <div className="flex items-center">
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex justify-center">
                <nav className="flex items-center space-x-2">
                  ...
                  {/* Page numbers */}
                  {(() => {
                    const pagesToShow = 10; // Show 10 page numbers at once
      ...
      {/* Floating Filter Button - Only on Properties page */}
